            ...
            >
                {options.map((o) => (
                    <option
                        key={o.value}
                        value={o.value}
                        className="text-base text-foreground bg-background"
                    >
                        {o.label}
                    </option>
                ))}
    ...
    return (
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
            <div className="lg:col-span-3 space-y-12">
                ...
                />
                <SoftInput
                    label="Average Distance per Vehicle"
                    unit="km/yr"
                    value={avgKm}
                    onChange={setAvgKm}
                    placeholder="12000"
                />
                <SoftInput
                    label="Fuel Economy (Consumption rate)"
                    unit="L / 100km"
                    value={fuelEconomy}
                ...
                <SoftSelect
                    ...
                    options={[
                        {
                            value: "GASOLINE",
                            label: `Gasoline (${EMISSION_FACTORS.GASOLINE} kgCO₂/L)`,
                        },
                        {
                            value: "DIESEL",
                            label: `Diesel (${EMISSION_FACTORS.DIESEL} kgCO₂/L)`,
                        },
                        {
                            value: "B35",
                            label: `Biodiesel B35 (${EMISSION_FACTORS.B35} kgCO₂/L)`,
                        },
                    ]}
                />
            </div>
...
export default function CalculatorPage() {
    ...
    const runRoad = () =>
        setResult(
            calculateRoadEmission(
                parseFloat(roadVehicles) || 0,
                parseFloat(roadKm) || 0,
                parseFloat(roadEcon) || 0,
                roadFuel,
            ),
        );
    const runRail = () =>
        setResult(
            calculateRailEmission(
                parseFloat(railEnergy) || 0,
                railTraction,
                railTraction === "Electric" ? "GRID_ELECTRICITY" : "DIESEL",
            ),
        );
    const runSea = () =>
        setResult(
            calculateSeaEmission(
                parseFloat(seaDist) || 0,
                parseFloat(seaIntensity) || 0,
                seaFuel,
            ),
        );
    const runAvi = () =>
        setResult(
            calculateAviationEmission(
                parseFloat(aviFlights) || 0,
                parseFloat(aviFuel) || 0,
            ),
        );
        ...
        <div className="max-w-5xl mx-auto pb-12">
            <div className="mb-12 text-center max-w-2xl mx-auto">
                ...
                <p className="text-lg text-muted-foreground">
                    Quickly estimate CO&sub2; emissions for specific scenarios without
                    ...
                    {activeTab === "road" && (
                        <RoadFormWrapper
                            ...
                            calculate={{
                                ...
                                setVehicles: setRoadVehicles,
                                avgKm: roadKm,
                                setAvgKm: setRoadKm,
                                fuelEconomy: roadEcon,
                                setFuelEconomy: setRoadEcon,
                                fuelCode: roadFuel,
                                setFuelCode: setRoadFuel,
                                run: runRoad,
                            }}
                        />
                    )}
                        ...
                        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
                            <div className="lg:col-span-3 space-y-12">
                                ...
                                <div className="p-4 rounded-xl bg-muted/50 text-sm text-muted-foreground border border-border/50">
                                    ...
                                    {railTraction === "Electric" ? "kgCO₂/kWh" : "kgCO₂/L"}
                                </div>
                            </div>
                            <div className="lg:col-span-2">
                    ...
                    {activeTab === "sea" && (
                        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
                            <div className="lg:col-span-3 space-y-12">
                                ...
                                />
                                <SoftInput
                                    label="Average Fuel Intensity"
                                    unit="tons per NM"
                                    value={seaIntensity}
                                    onChange={setSeaIntensity}
                                    placeholder="0.015"
                                />
                                <SoftSelect
                                    ...
                                    options={[
                                        {
                                            value: "MARINE_DIESEL",
                                            label: `Marine Diesel (${EMISSION_FACTORS.MARINE_DIESEL} tCO₂/t)`,
                                        },
                                        {
                                            value: "HFO_LFO",
                                            label: `Heavy/Light Fuel Oil (${EMISSION_FACTORS.HFO_LFO} tCO₂/t)`,
                                        },
                                        {
                                            value: "LNG_LPG",
                                            label: `LNG / LPG (${EMISSION_FACTORS.LNG_LPG} tCO₂/t)`,
                                        },
                                    ]}
                                />
                            </div>
                        ...
                        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
                            <div className="lg:col-span-3 space-y-12">
                                <SoftInput
                                    ...
                                    value={aviFlights}
                                    onChange={setAviFlights}
                                    placeholder="10000"
                                />
                                <SoftInput
                                    label="Average Fuel per Flight"
                                    unit="Liters"
                                    value={aviFuel}
                                    onChange={setAviFuel}
                                    placeholder="5000"
                                ...
                                <div className="p-4 rounded-xl bg-muted/50 text-sm border border-border/50">
                                    <p className="text-foreground font-medium mb-1">
                                        Standard Aviation Constants
                                    </p>
                                    <ul className="text-muted-foreground list-disc pl-4 space-y-1">
                                        <li>
                                            Fuel density:{" "}
                                            <strong>{AVIATION_FUEL_DENSITY} kg/L</strong>
                                        </li>
                                        <li>
                                            Jet Fuel EF:{" "}
                                            <strong>{EMISSION_FACTORS.JET_FUEL} kgCO₂/kg</strong>
                                        </li>
                                    </ul>
                                </div>
                            </div>
